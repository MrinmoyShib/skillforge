...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const usernameError = validate(formData.username, validators.required);
    const passwordError = validate(formData.password, validators.required);
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-12rem)] px-4">
      <div className="w-full max-w-md bg-[var(--color-surface-card)] border border-[var(--color-surface-hover)] rounded-2xl p-8 shadow-2xl">
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl bg-[var(--color-surface-dark)] border ${errors.password ? 'border-red-500 ring-1 ring-red-500' : 'border-[var(--color-surface-hover)] focus:border-[var(--color-brand-primary)] focus:ring-1 focus:ring-[var(--color-brand-primary)]'
                  } text-white placeholder-slate-500 text-sm transition outline-none pr-11`}
              />
              <button
                type="button"
        ...
        <div className="mt-8 pt-6 border-t border-[var(--color-surface-hover)] text-center text-sm text-[var(--color-text-secondary)]">
          ...
          <Link to="/register" className="text-[var(--color-brand-accent)] hover:underline font-medium">
            Create an account
          </Link>
        </div>
      </div>
